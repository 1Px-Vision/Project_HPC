...
cdef extern from "digitrevorder_kernel.cuh":
    void digitrevorder_kernel(int *x, int *vec, int *result, int N, int L, int base)

cdef extern from "fhtseq_inv_gpu_kernel.cuh":
    void fhtseq_inv_gpu_kernel(float *x, int N, int L)
...
def digitrevorder(np.ndarray[np.int32_t, ndim=1] x, int base):
    ...
    cudaMalloc(<void **>&d_vec, L * sizeof(int))
    ...
    digitrevorder_kernel(d_x, d_vec, d_result, N, L, base)
