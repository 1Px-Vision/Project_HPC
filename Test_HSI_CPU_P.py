...
def getHSPIReconstructionThread():
  global result, spec
  result, spec = getHSPIReconstruction(intensity_mat, nStep)

def getHSPIReconstructionThread_1(args):
    intensity_mat, nStep = args
    return getHSPIReconstruction(intensity_mat, nStep)
cmap='gray');
    plt.title("CPU-OMP")


if __name__ == "__main__":
    # Load data and other setup
    m_parameter = scipy.io.loadmat('/content/parameter_hsi.mat')
    m_image = scipy.io.loadmat('/content/dat_HSI_64.mat')
    ...
    # Create a multiprocessing process to run getHSPIReconstruction
    result = None
    spec = None
    
    P=4
    args_list = [(intensity_mat, nStep)] * P
    with mp.Pool(processes=P) as pool:
        results = pool.map(getHSPIReconstructionThread_1, args_list)
    ...
    time_pro = end - start
    print('Time HSI_CPU={} ms'.format(time_pro * 1000))
